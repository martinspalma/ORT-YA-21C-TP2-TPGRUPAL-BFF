...
export async function enfrentarCartas(sala, persistencia, usuarioServicio) {
  ...
  if (ganador === 'empate') {
        ...
        await usuarioServicio.actualizarEstadisticas(j1.id, { draws: 1 }); // Usamos 'draws' directamente
        await usuarioServicio.actualizarEstadisticas(j2.id, { draws: 1 });
        ...
        if (winnerId) await usuarioServicio.actualizarEstadisticas(winnerId, { wins: 1 });
        if (loserId) await usuarioServicio.actualizarEstadisticas(loserId, { losses: 1 });
  ...
  return {
    ...
    ganador,
    resultadoRonda: sala.resultado // Puedes devolver el resultado de la ronda para mostrarlo
  }
